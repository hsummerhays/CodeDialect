...
public static class InitialSeed
{
    public static async Task SeedAsync(IServiceProvider serviceProvider)
    {
        ...
        var apiOld = new ChallengeImplementation { Challenge = apiChallenge, Dialect = netFramework, StarterCode = "public class UsersController : ApiController {\n    [HttpGet]\n    public IHttpActionResult Get() { ... }\n}", ReferenceSolution = "public class UsersController : ApiController {\n    [Route(\"api/users\")]\n    public IHttpActionResult GetUsers() {\n        return Ok(new { Name = \"John\" });\n    }\n}" };
        var apiNew = new ChallengeImplementation { Challenge = apiChallenge, Dialect = net10, StarterCode = "var app = builder.Build();\n// Map your endpoint here", ReferenceSolution = "app.MapGet(\"/api/users\", () => new { Name = \"John\" });" };
        ...
        var java8Concurrency = new ChallengeImplementation { Challenge = concurrencyChallenge, Dialect = java8, StarterCode = "ExecutorService executor = Executors.newFixedThreadPool(100);", ReferenceSolution = "ExecutorService executor = Executors.newFixedThreadPool(100);\nfor (int i = 0; i < 10000; i++) {\n    executor.submit(() -> {\n        Thread.sleep(1000);\n        return \"Done\";\n    });\n}" };
        var java21Concurrency = new ChallengeImplementation { Challenge = concurrencyChallenge, Dialect = java21, StarterCode = "try (var executor = Executors.new...) { }", ReferenceSolution = "try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {\n    IntStream.range(0, 10000).forEach(i -> {\n        executor.submit(() -> {\n            Thread.sleep(1000);\n            return \"Done\";\n        });\n    });\n}" };
        ...
        var jsLegacyIo = new ChallengeImplementation { Challenge = fileIoChallenge, Dialect = jsLegacy, StarterCode = "fs.readFile('data.json', function(err, data) { ... });", ReferenceSolution = "fs.readFile('data.json', 'utf8', function(err, data) {\n  if (err) throw err;\n  var obj = JSON.parse(data);\n  console.log(obj.name);\n});" };
        var jsModernIo = new ChallengeImplementation { Challenge = fileIoChallenge, Dialect = jsModern, StarterCode = "const data = await fs.readFile(...);", ReferenceSolution = "import fs from 'fs/promises';\nconst data = await fs.readFile('data.json', 'utf8');\nconst { name } = JSON.parse(data);\nconsole.log(name);" };
        ...
        var diOld = new ChallengeImplementation { Challenge = diChallenge, Dialect = netFramework, StarterCode = "public class UserService {\n    private readonly IUserRepository _repo;\n    public UserService() { ... }\n}", ReferenceSolution = "public class UserService {\n    private readonly IUserRepository _repo;\n    public UserService(IUserRepository repo) {\n        _repo = repo;\n    }\n}" };
        var diNew = new ChallengeImplementation { Challenge = diChallenge, Dialect = net10, StarterCode = "public class UserService(...) { }", ReferenceSolution = "public class UserService(IUserRepository repo) {\n    public async Task<User> Get() => await repo.GetAsync();\n}" };
        ...
        var tsSafetyImpl = new ChallengeImplementation { Challenge = typeSafeChallenge, Dialect = tsModern, StarterCode = "type ApiResponse = Success | Error;", ReferenceSolution = "interface Success { status: 'success'; data: string; }\ninterface Error { status: 'error'; message: string; }\ntype ApiResponse = Success | Error;\n\nfunction handle(res: ApiResponse) {\n  if (res.status === 'success') {\n    console.log(res.data);\n  } else {\n    console.error(res.message);\n  }\n}" };
        ...
        var py2Impl = new ChallengeImplementation { Challenge = pythonDataChallenge, Dialect = python2, StarterCode = "project = {'name': 'Legacy', 'days': 10}", ReferenceSolution = "project = {'name': 'CodeDialect', 'deadline': '2026-12-31'}\nprint 'Project:', project['name']" };
        var py3Impl = new ChallengeImplementation { Challenge = pythonDataChallenge, Dialect = python3, StarterCode = "@dataclass\nclass Project: ...", ReferenceSolution = "from dataclasses import dataclass\n@dataclass\nclass Project:\n    name: string\n    deadline: datetime\n\nproj = Project('CodeDialect', datetime.now())" };
